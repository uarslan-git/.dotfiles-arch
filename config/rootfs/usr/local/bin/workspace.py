...
GAME_CLASSES = ['steam', 'dota2', 'cs2', 'hl2']  # Add more game classes as needed
...
class MonitorManager:
    ...
    def update_monitor_info(self):
        ...
        # Check if any game is active on workspace 10
    ...
    def bounce_window_if_needed(self, window):
        ...
        if not target_workspace:
            target_workspace = workspace_vars['ws1']  # Default workspace
    ...
    def ensure_workspace_on_correct_monitor(self, workspace_name):
        ...
        # Workspace 10 always stays on primary monitor
        if workspace_name == workspace_vars['ws10']:
            target_monitor = self.primary_monitor
        else:
            # When gaming, everything else goes to secondary
            target_monitor = self.secondary_monitor if self.gaming_mode else self.primary_monitor
...
def on_window_new(i3, e, manager):
    ...
    # Update monitor info in case gaming status changed
    manager.update_monitor_info()
    
    # First check if we need to bounce this window
    if manager.bounce_window_if_needed(window):
        return
    
    # Normal window assignment
